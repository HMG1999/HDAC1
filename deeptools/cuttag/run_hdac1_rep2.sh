...
# =========================
# 输入文件（D16_D19_HDAC1_rep2: D16 vs D19）
# =========================
BASE="/mnt/My_disk/Li_linghang/liumeng_HDCAC1/Cut_Tag/D16_D19_HDAC1_rep2"
BAM1="$BASE/D16_Hdac1Aligned.sortedByCoord.out.bam"
BAM2="$BASE/D19_Hdac1Aligned.sortedByCoord.out.bam"
...
OUTDIR="/mnt/My_disk/Li_linghang/liumeng_HDCAC1/Cut_Tag/cuttag_pipeline_1/HDAC"
...
"$DT/plotHeatmap" \
  ...
  --plotTitle "D16_D19_HDAC1_rep2: union peaks signal" \
...
PY
  ...
  "$DT/plotHeatmap" \
    ...
    --colorList "white,#3376CD" \
    --plotTitle "D16_D19_HDAC1_rep2: signal groups |log2FC| >= ${cutoff}" \
  ...
  if [[ -s "$OUTDIR/beds/gained_lfc_${tag}.bed" && -s "$OUTDIR/beds/lost_lfc_${tag}.bed" ]]; then
    ...
    "$DT/plotHeatmap" \
      ...
      --colorList "white,red" \
      --plotTitle "D16_D19_HDAC1_rep2: Gained vs Lost |log2FC| >= ${cutoff} (custom)" \
      --zMin 0 --zMax auto --interpolationMethod bilinear \
      --sortRegions descend --sortUsing mean \
      --missingDataColor white --whatToShow "plot, heatmap and colorbar" \
