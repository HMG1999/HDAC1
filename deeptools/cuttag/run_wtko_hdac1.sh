...
# =========================
# 输入文件（HDAC1_WT_KO_rep1: WT vs KO）
# =========================
BASE="/mnt/My_disk/Li_linghang/liumeng_HDCAC1/Cut_Tag/HDAC1_D16_D19_WT_KO_rep1"
BAM1="$BASE/WT-Hdac1Aligned.sortedByCoord.out.bam"
BAM2="$BASE/KO-Hdac1Aligned.sortedByCoord.out.bam"
...
OUTDIR="/mnt/My_disk/Li_linghang/liumeng_HDCAC1/Cut_Tag/cuttag_pipeline_1/WTKO"
...
"$DT/plotHeatmap" \
  ...
  --plotTitle "WT_KO_HDAC1: union peaks signal" \
...
"$DT/bigwigCompare" \
  ...
  --numberOfProcessors "$THREADS" \
  --outFileFormat bigwig \
  --outFileName "$OUTDIR/bigwig/KO_vs_WT.log2fc.pseudo${PSEUDO}.bw"
...
PY
  ...
  "$DT/plotHeatmap" \
    ...
    --colorList "white,#3376CD" \
    --plotTitle "WT_KO_HDAC1: signal groups |log2FC| >= ${cutoff}" \
  ...
  if [[ -s "$OUTDIR/beds/gained_lfc_${tag}.bed" && -s "$OUTDIR/beds/lost_lfc_${tag}.bed" ]]; then
    ...
    "$DT/plotHeatmap" \
      ...
      --colorList "white,red" \
      --plotTitle "WT_KO_HDAC1: Gained vs Lost |log2FC| >= ${cutoff} (custom)" \
      --zMin 0 --zMax auto --interpolationMethod bilinear \
      --sortRegions descend --sortUsing mean \
      --missingDataColor white --whatToShow "plot, heatmap and colorbar" \
